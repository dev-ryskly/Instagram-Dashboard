...
export default function ConversionChart({ reels }: ConversionChartProps) {
  if (reels.length === 0) {
    return (
      <div className="card">
        <p className="text-muted">No conversion data yet.</p>
      </div>
    )
  }
  ...
  const data: ChartEntry[] = reels.map((reel) => ({
    ...
    rate: parseFloat((reel.conversion_rate * 100).toFixed(2)),
  }))
